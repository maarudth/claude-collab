...
- **Cancel:** `User clicked CANCEL` → stop immediately, ask what they want via `design_chat`.
- **Message:** `[COLLAB] User said: ...` → direction change: stop and respond. Minor feedback ("make it blue"): acknowledge via `design_chat`, continue with it applied.
...
- Lifecycle: start it (background Bash, `run_in_background: true`) → it runs until it delivers one message → it exits → you respond via `design_chat` → **immediately start a new one** (same command, provided in `[COLLAB-SETUP]`).
...
- After `design_close`, `[COLLAB-CLEANUP]` appears — do NOT start a new listener.

**Speed matters when a message lands:** the user is watching the widget. Respond via `design_chat` first (short is fine), restart the listener in the same message (parallel tool call), then continue working.
...
2. When they send a message, selections arrive automatically as `[COLLAB] Selected elements: [...]` — no need to call `design_selections`.
3. Call `design_selections` only if they selected elements but haven't messaged yet.
...
1. `design_browse` URL → 2. `design_scan` (snapshot) → 3. `design_act` with batched `steps[]` → 4. re-scan after DOM changes (refs go stale) → 5. screenshot only for visual verification.

### Design iteration
Mock up with `design_preview` → user approves → apply with `design_evaluate` → verify with scan (screenshot only if visual) → critique → fix → repeat.

### Inspiration → build
Browse references in tabs → user clicks elements they like → `design_collect` with category → `design_moodboard` to review → `design_synthesize` (use `clear: true` after) → build from the synthesis output.

### Preview vs evaluate
"Look at this" (mockups, variants, reports) → `design_preview`. "Change the page" (DOM edits, builds, reading state) → `design_evaluate`.

## 3. Per-Tool Gotchas

### design_evaluate
- **Before significant DOM manipulation** (replacing body content, building full pages), call `design_inbox` first — the user may have sent "wait"/"stop" that hasn't reached you. Check between steps of multi-step plans.
...
### design_act / design_scan
- Batch interactions into one `design_act` call via `steps[]` (form fill = click + type + click submit in ONE call). Add `delay` when the page needs to react.
...
- Not yet supported: `design_wireframe`, element/full-page screenshots (viewport only).
- Auth: `design_browse` returns port + token when the extension isn't connected — relay them to the user to paste into the extension popup (one-time per session). Stale tokens auto-clear.

### Voice
- Track the last "Voice mode ON/OFF" system message — it determines whether you speak. ON → both `design_voice_tts` and `design_chat`. OFF → chat only, stop speaking immediately.

## 4. Performance

- Every tool call is a full round trip — minimize calls: scan over screenshot, batched act, batched evaluate.
- Chat history caps at 1000 messages (oldest trimmed) — `design_export_chat` preserves long sessions; do it before context gets long.
...
- Close cleanly with `design_close` when the session ends.
